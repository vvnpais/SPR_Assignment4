...
def plot_decision_boundary(X_train, y_train, classifier, K, resolution=0.02):
    # Define the min and max bounds for the grid
...
def evaluate_model(y_true, y_pred):
    ...
    f1_scores = {label: metrics['f1-score'] for label, metrics in class_report.items() if label != 'accuracy'}
...
# Perform classification with different values of K
...
for K in K_vals:
    ...
    confusion_matrices[int(K)] = conf_matrix.tolist()  # Convert to list for JSON serialization
