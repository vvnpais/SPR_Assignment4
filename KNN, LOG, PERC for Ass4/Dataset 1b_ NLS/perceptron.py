    ...
    # Ensure inputs are NumPy arrays
        ...
        for i in range(len(classes)):
            for j in range(i + 1, len(classes)):
                ...
                X_subset = X_train[indices]  # Subset the training data
                y_subset = y_train[indices]  # Subset the labels

                # Relabel to +1 and -1 for binary classification
                y_binary = np.where(y_subset == class_i, 1, -1)

                # Train perceptron for this pair
